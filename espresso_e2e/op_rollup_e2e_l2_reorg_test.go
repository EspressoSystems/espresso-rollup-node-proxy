...
func TestOPE2EL2Reorg(t *testing.T) {
	...
	previous := monitorStoredBlockProgress(t, espressoStore, blockBeforeReorg, 4*time.Minute, opGethFullNode, func(current uint64) bool {
		return current > currentSeqBlock+20
	})

	require.Greater(t, previous, currentSeqBlock+20,
