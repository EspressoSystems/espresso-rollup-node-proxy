...
func runProxyRPCCompatibility(t *testing.T, proxyURL, directURL string, store *espressostore.EspressoStore, freeze func()) {
	t.Helper()
	...
	for _, tc := range methodsWithNoEspressoTag {
		t.Run(tc.method, func(t *testing.T) {
			...
			directResp := jsonRPCCallRaw(t, directURL, tc.method, jsonMarshal(t, tc.params))
	...
	// Freeze the store so the espresso tag resolves to a stable block.
	freeze()
	...
	for _, tc := range espressoTagMethods {
		t.Run(tc.method, func(t *testing.T) {
			...
			directResp := jsonRPCCallRaw(t, directURL, tc.method, directParams)
			requireJSONRPCEqual(t, directResp, proxyResp, tc.method)
		})
	}
	...
	t.Run("batch_request", func(t *testing.T) {
		...
		directResps := jsonRPCBatchCallRaw(t, directURL, directBatch)
