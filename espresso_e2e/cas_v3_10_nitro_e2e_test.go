...
func TestCASV3_10NitroE2ERollupEspressoProxy(t *testing.T) {
	ctx := context.Background()

	t.Log("Starting CAS-backed Nitro v3.10 stack (l1-anvil, espresso-dev-node, sequencer, full-node, tx-generator)")
	shutdown := runDockerCompose(casWorkingDirV3_10, "l1-anvil", "espresso-dev-node", "sequencer", "full-node", "tx-generator")
	defer shutdown()
	...
	t.Log("Starting CAS container (v3.10 image)")
	dockerComposeStart(t, casWorkingDirV3_10, nil, "cas")
	waitForCasReady(t, 2*time.Minute)

	t.Log("Starting Nitro poster (DA = CAS calldata)")
	dockerComposeStart(t, casWorkingDirV3_10, nil, "poster")

	espressoStore := newTestStore(t, "cas-v3_10-nitro-espresso-state", 0)
	...
	v := startCasNitroVerifier(ctx, t, espressoStore, casBridgeAddressV3_10)
	defer v.Stop()
